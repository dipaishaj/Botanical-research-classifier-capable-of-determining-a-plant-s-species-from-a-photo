...
train_path ="./Plant_Seedling_Data/train"
test_path = "./Plant_Seedling_Data"

for dirpath, dirnames, filenames in os.walk("./Plant_Seedling_Data"):
...
"""**Before, we proceed further, it's important to know the dimensions of these images. 
...
"""**It is observed that the original images have height and width of 150 and 
third dimension 3 indicates that the images are of 3 channels i.e. RGB color images.**
...
therefore, we should establish a base size for all images fed into our AI algorithms by resizing them.**
"""
...
"""**Not all the species have equal number of images. 
The plant species 'Maize' has the highest number of images around 680 images roughly. 
Whereas, the 'Black Grass' plant species has the least number of images with around 220 images.
...
* PCA is a way of linearly transforming the data such that most of the information in the data 
is contained within a smaller number of features called components.
* For performing PCA for colored images, the images have to be flattened and then split into 
their red, green and blue channels.
...
Random forest, as its name implies, consists of a large number of individual decision trees that 
operate as an ensemble. Each individual tree in the random forest declares a class prediction and 
...
* During this supervised phase, the network compares its actual output produced with what 
it was meant to produce—the desired output.
* The difference between both outcomes is adjusted using backpropagation. 
This means that the network works backward, going from the output unit to the input units to adjust 
...
plot_model(ann, to_file='/content/drive/MyDrive/AIML-GL/Computer Vision/CV PROJECT/model_ann.png',
...
"""**We can see that training accuracy of the model is 65% while validation accuracy is less. 
...
"""### **The ANN model has an accuracy of 0.64 approximately and loss of 1.62 approximately. The loss is high. 
It can be reduced by modifying the ANN model by adding more dense layers and neurons. 
We can also try various learning rates of the optimizer.**
...
checkpoint = tf.keras.callbacks.ModelCheckpoint('./plant_classifier.h5',save_best_only=True, monitor='val_accuracy',mode='max', verbose = 1)

"""We'll train the CNN model with train data and validate the model using validation data. 
We have set the number of epochs as 20.Number of steps per epoch are set equal to length of training data."""
...
The ImageDataGenerator() from Keras provides a quick and easy way to augment your images. 
...
However, the main benefit of using the Keras ImageDataGenerator class is that it is designed 
to provide real-time data augmentation. ImageDataGenerator class ensures that the model 
receives new variations of the images at each epoch. But it only returns the transformed images 
and does not add it to the original corpus of images. If it was, in fact, the case, 
then the model would be seeing the original images multiple times which would definitely overfit our model.
...
"""Here we can observe that CNN model with data augmentation has worked best on 
predicting the image with highest 73% test accuracy.
The plot shows validation accuracy as well as loss is fairly mapping training 
accuracy and training loss respectively.
...
"""Thus using augmented images, the CNN model has a better accuracy of 81% approximately and with a lesser 
loss than CNN without data augmentation. Since, the CNN model with augmented data has a better performance 
than all the models, we'll save this model.
"""
...
Generally, the models are made too wide, deep, or with a very high resolution. Increasing these characteristics helps 
the model initially but it quickly saturates and the model made just has more parameters and is therefore not efficient. 
...
**As the input shape of transfer learning model requires image of shape 224 by 224, 
resizing the test image for prediction.**
"""
...
on the best performing algorithm and why/how it outperforms other algorithms in this case**.
* Convolution Neural Networks outperforms Neural Networks and which in turn out perform Supervised 
Learning models.

* The best part of CNNs is high accuracy of image prediction and less loss. Also, we do not have to do 
feature extraction. The system learns to do feature extraction and the core concept of CNN is, it uses 
convolution of image and filters to generate invariant features which are passed on to the next layer.
* The features in the next layer are convoluted with different filters to generate more invariant and 
abstract features and the process continues till one gets final feature / output which is invariant to occlusions.

* ANNs are better than Supervised Learning models because we don't have to perform feature extraction in ANNs 
whereas in Supervised Learning, we must perform the Principal component analysis (PCA) to compute the principal 
components in images.
* In predicting the Plant Seedlings dataset, we can see that the accuracy of Random Forest and Support Vector 
Machines models are less than 20%.
* Using ANNs, the accuracy increased to 45%. This was improved much more by using CNNs. 
The accuracy increased to approximately 76% with less loss. CNNs with Data Augmentation has the highest accuracy 
of 87%.

* ### It can clearly be observed that ***CNN WITH TRANSFER LEARNING*** **outperformed** over all the models with 
**accuracy score of 89%** and **loss of 0.47**.
...
"""## **5.** **Import the the test image** **[from the “ Prediction” folder]** **to predict the class. 
Display the image. Use the best trained image classifier model to predict the class.**
## **Use the best model to predict the data**

**Let's use the pickle model for prediction and check if the prediction is same as that of prediction 
made by model before pickling.**
"""
...
"""**The original classifier and pickled classifier are the same. This can be proved by comparing the 
prediction achieved by both of them on the same testing set. When compared, we are returned True, which 
shows the prediction are the same and proves that the classifiers are the same.**
